<template>
  <section class="section px-6 py-12 md:px-12">
    <div class="container mx-auto text-left text-sm">
      <h1 class="mb-4 text-xl font-bold">共通ルール</h1>
      <div>
      ...
      <h2 class="mt-6 mb-3 text-lg font-bold">村ごとに明記がない限り以下も守ってください</h2>
      <div>
        <ul class="ml-4 list-disc space-y-2">
      ...
      <h3 class="mt-6 mb-3 text-lg font-bold">村作成</h3>
      <div>
        <ul class="ml-4 list-disc space-y-2">
          <li>
            プレイヤーが村を作成できます。
      ...
      <h3 class="mt-6 mb-3 text-lg font-bold">村建てが行えること</h3>
      <div>
        <ul class="ml-4 list-disc space-y-2">
      ...
      <div>
        <ul class="ml-4 list-disc space-y-2">
          <li>2日目から投票による処刑が始まります。<br />（最初に処刑死者が出るのは3日目朝）</li>
      ...
      <h3 class="mt-6 mb-3 text-lg font-bold">時短希望</h3>
      <div>
        <ul class="ml-4 list-disc space-y-2">
      ...
      <div>
        <ul class="ml-4 list-disc space-y-2">
          <li>進行中の通常発言ができる時間帯に生存者は役職カミングアウトをすることができます。</li>
          <li>
      ...
      <h3 class="mt-6 mb-3 text-lg font-bold">アクション</h3>
      <div>
        <ul class="ml-4 list-disc space-y-2">
          <li>
      ...
      <div>
        <ul class="ml-4 list-disc space-y-2">
          <li>
            ...
            <ul class="my-2 ml-6 list-disc">
              <li>ただし、時短機能により日付更新された場合は発言していなくても突然死しません。</li>
            </ul>
          </li>
      ...
      <h3 class="mt-6 mb-3 text-lg font-bold">ステータス</h3>
      <div>
        <ul class="ml-4 list-disc space-y-2">
          <li>
            恋絆
      ...
      <h3 class="mt-6 mb-3 text-lg font-bold">勝敗判定</h3>
      ...
      <div class="mt-2 mb-5 overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-300">
          <thead class="bg-gray-50">
            <tr>
              <th class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">前提</th>
              <th class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">勝利条件</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-200 bg-white">
            <tr v-for="(condition, index) in personalWinConditions" :key="index">
      ...
      <div class="mt-2 mb-5">
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-300">
            <thead class="bg-gray-50">
              <tr>
                <th class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">陣営</th>
                <th class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">勝利条件</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-200 bg-white">
        ...
        <ul class="ml-4 list-disc space-y-2">
          ...
          <li>
            ...
            <ul class="my-2 ml-6 list-disc">
              <li>
                [[cw]]文字列[[/cw]] →
                <span class="netabare cursor-pointer" @click="removeNetabare($event)">文字列</span>
              </li>
            </ul>
          </li>
        </ul>
...
const camps = [
  ...
  {
    name: "人狼陣営",
    win_condition: "夜明け時点で生存している人狼カウント数 ≧ 生存している人間カウント数",
  },
  {
    name: "村人陣営",
    win_condition:
...
onMounted(async () => {
  try {
    const response = await $fetch<{ list: Skill[] }>(`${config.public.apiBaseUrl}/skill/list`);
...
// 恋絆ステータスメッセージ例
const loverStatusMessage = computed<MessageView>(() => {
  ...
  const content: MessageContent = {
    type: messageType,
    text: "村長 ヴァルターは、絆に引きずられるように農夫 ヤコブの後を追った。",
    is_strong: false,
  };
...
useSeoMeta(
  createSeoMeta({
    title: "ルール",
    description:
      "LASTWOLFの人狼ゲームルールを詳しく説明します。村作成、役職、投票、発言などのゲームシステムについて。",
    keywords: "LASTWOLF,人狼,ルール,役職,投票,発言,長期人狼,ゲームルール",
    ogType: "article",
  }),
);
