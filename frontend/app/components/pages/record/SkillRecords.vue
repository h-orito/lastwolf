<template>
  <div>
    <!-- データなし -->
    <div v-if="skillRecords.length === 0" class="py-4 text-center text-gray-500 text-sm">
      <p>参加した村がありません</p>
    </div>
    ...
    <div v-else class="overflow-x-auto">
      <table class="w-full border-collapse bg-white text-xs whitespace-nowrap">
        ...
        <tbody>
          <tr
            v-for="record in skillRecords"
            :key="record.skill.code"
            class="odd:bg-white even:bg-gray-50"
          >
            <td class="border border-gray-300 px-3 py-1">{{ record.skill.name }}</td>
