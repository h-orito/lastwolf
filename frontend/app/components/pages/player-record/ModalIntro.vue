<template>
  <Modal v-model="isOpen" title="自己紹介編集">
    <div class="space-y-4">
      ...
      <!-- ニックネーム -->
      <div>
        <label class="mb-1 block text-sm font-semibold text-gray-700"> ニックネーム </label>
        <p class="mb-2 text-xs text-gray-500">エピローグで全員に公開されます。</p>
        <FormInput v-model="nickname" size="sm" :maxlength="50" :error="!validNickname" />
        <p v-if="!validNickname" class="mt-1 text-xs text-red-500">50字以内で入力してください</p>
      </div>

      <!-- 他人狼サイトのID -->
      <div>
        <label class="mb-1 block text-sm font-semibold text-gray-700"> 他人狼サイトのID </label>
        <p class="mb-2 text-xs text-gray-500">
          エピローグまでにこのIDを入力しておくと、戦績サイトにこのIDで登録されます。
        </p>
        <FormInput v-model="otherSiteName" size="sm" :maxlength="20" :error="!validOtherSiteName" />
      ...
      <!-- 自己紹介 -->
      <div>
        <label class="mb-1 block text-sm font-semibold text-gray-700"> 自己紹介 </label>
    ...
    <template #footer>
      <UiButton color="secondary" size="sm" @click="close"> キャンセル </UiButton>
      <UiButton
        color="primary"
        size="sm"
...
const validOtherSiteName = computed(() => !otherSiteName.value || otherSiteName.value.length <= 20);
const introLength = computed(() => intro.value?.length ?? 0);
const validIntro = computed(() => introLength.value <= 2000);
const canSave = computed(() => validNickname.value && validOtherSiteName.value && validIntro.value);
