    ...
    <div class="mb-6">
      ...
      <div v-if="selectedChara" class="mb-3">
        ...
        <div class="mb-1 text-left text-xs font-bold">
          [{{ formData.dummyCharaShortName || selectedChara.name.short_name }}]
          {{ formData.dummyCharaName || selectedChara.name.name }}
        </div>
        <!-- 画像とテキストエリア -->
      ...
      <p v-if="!day0Readonly && errors?.day0Message" class="mt-1 text-xs text-red-600">
        {{ errors.day0Message }}
      </p>
      <p v-else-if="!day0Readonly" class="mt-1 text-xs text-gray-500">
        最大1000文字まで入力できます（{{ (formData.day0Message || "").length }}/1000）
      </p>
    </div>

    <!-- 1日目発言 -->
    <div>
      <label class="mb-2 block text-sm font-medium text-gray-700"> 1日目の発言内容 </label>
      ...
      <div v-else>
        <p class="mt-1 text-xs text-gray-500">
          最大1000文字まで入力できます（{{ (formData.day1Message || "").length }}/1000）
        </p>
      </div>
    </div>
  </div>
