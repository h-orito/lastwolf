  ...
  <Modal v-model="isOpen" title="他SNSアカウント連携" @close="close">
    <div class="content">
      ...
      <Alert type="warning" class="my-4 text-sm">
        <ul class="list-disc">
          <li>ログインしたことがあるアカウントで追加ログインすることはできません。</li>
        </ul>
      </Alert>

      <div class="mt-6 mb-6">
        <hr class="mb-3" />
        <UiButton color="primary" :disabled="isAlreadyTwitterLinked" @click="link('twitter')">
      ...
      <div class="my-6">
        <hr class="mb-3" />
        <UiButton color="primary" :disabled="isAlreadyGoogleLinked" @click="link('google')">
    ...
    <template #footer>
      <UiButton color="secondary" variant="outline" size="sm" @click="close"> 閉じる </UiButton>
    </template>
  </Modal>
</template>
...
const isAlreadyTwitterLinked = computed(() => {
  ...
  return (
    user.providerData?.some((providerData) => providerData.providerId === "twitter.com") ?? false
  );
});
...
const isAlreadyGoogleLinked = computed(() => {
  ...
  // Firebaseのユーザー情報からプロバイダー情報を取得
  return (
    user.providerData?.some((providerData) => providerData.providerId === "google.com") ?? false
  );
});
