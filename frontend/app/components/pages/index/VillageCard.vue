...
const latestday = computed(() => {
  const dayList = props.village.days?.list || [];
...
const daychangeDatetime = computed(() => {
  ...
  if (statusCode === VILLAGE_STATUS.COMPLETED || statusCode === VILLAGE_STATUS.CANCEL) {
    return null;
  }
  const datetime = latestday.value?.end_datetime;
  return datetime ? datetime.substring(0, 16) : null;
});

// 参加者数表示
const participantStatus = computed(() => {
  const participantCount = props.village.participants?.count || 0;
...
const sayableTime = computed(() => {
  const silentSeconds = props.village.setting?.rules?.silent_seconds;
  if (!silentSeconds) return "24時間";
  const silentHours = Math.floor(silentSeconds / 3600);
  if (!silentHours) return "24時間";
  return `${24 - silentHours}時間`;
});
...
// 年齢制限（LASTWOLFにはタグがないため非表示）
const ageLimit = computed(() => null);
</script>
