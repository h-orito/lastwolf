<template>
  <div class="menu-area">
    ...
    <div v-else>
      ...
      <div v-if="!isLogin">
        <div class="mb-12">
          <p class="spotlight-shadow text-xl md:text-xl">アプリ連携すると参加できます</p>
        </div>
      ...
      <div v-else>
        <div class="spotlight-shadow mb-5">
          ...
          <p class="spotlight-shadow text-sm">ニックネームはマイページで変更できます</p>
        </div>
...
const participatingVillages = computed<SimpleVillageView[]>(() => {
  ...
  const epilogueVillages = (props.user.participate_finished_villages?.list || []).filter(
    (village: SimpleVillageView) => village.status?.code === VILLAGE_STATUS.EPILOGUE,
  );
