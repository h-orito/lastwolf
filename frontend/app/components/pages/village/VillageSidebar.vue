<template>
  <div>
    ...
    <!-- モーダル（仮実装） -->
    <ModalVillageInfo :is-open="isOpenVillageInfoModal" @close="closeVillageInfoModal" />
    <ModalUserSettings :is-open="isOpenUserSettingsModal" @close="closeUserSettingsModal" />
...
const personCount = computed(() => {
  ...
  return `${village.value.participants.count}`;
});
