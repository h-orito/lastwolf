...
describe("VillageFooter", () => {
  ...
  describe("モバイル表示", () => {
    it("モバイルでない場合、メニューボタンが表示されない", () => {
      ...
      const hasMenuButton = buttons.some((b) => b.attributes("aria-label") === "サイドバーを開く");
    ...
    it("メニューボタンをクリックすると toggleSlider が呼ばれる", async () => {
      ...
      const buttons = wrapper.findAll("button");
      const menuButton = buttons.find((b) => b.attributes("aria-label") === "サイドバーを開く");
  ...
  describe("更新ボタン", () => {
    it("更新ボタンをクリックすると refresh が呼ばれる", async () => {
      ...
      const buttons = wrapper.findAll("button");
      const refreshButton = buttons.find((b) => b.attributes("aria-label") === "発言を更新");
  ...
  describe("最下部スクロールボタン", () => {
    it("最下部スクロールボタンをクリックすると scrollToBottom が呼ばれる", async () => {
      ...
      const buttons = wrapper.findAll("button");
      const bottomButton = buttons.find((b) => b.attributes("aria-label") === "最下部にスクロール");
  ...
  describe("抽出ボタン", () => {
    it("フィルタリング中でない場合、抽出ボタンをクリックするとモーダルが開く", async () => {
      ...
      const buttons = wrapper.findAll("button");
      const filterButton = buttons.find((b) => b.attributes("aria-label") === "発言を抽出");
