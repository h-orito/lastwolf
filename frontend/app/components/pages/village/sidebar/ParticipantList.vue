<template>
  <div class="mb-4">
    ...
    <div v-if="spectatorList.length > 0">
      <div class="border-t border-gray-600 pt-1 pb-1">見学（{{ spectatorList.length }}人）</div>
      <div
...
const isProgress = computed(() => {
  ...
  return statusCode !== VILLAGE_STATUS.PROLOGUE && statusCode !== VILLAGE_STATUS.CANCEL;
});

const participantList = computed(
  () => (village.value?.participants.member_list ?? []) as VillageParticipantView[],
);
...
const spectatorList = computed(() => [] as VillageParticipantView[]);

// Methods
const compareParticipant = (vp1: VillageParticipantView, vp2: VillageParticipantView): number => {
  // 死亡している人より死亡していない人が先
...
const comingout = (participant: VillageParticipantView): string | null => {
  const co = participant.coming_out;
  if (!co) return null;
  return co.skill.short_name + "CO";
};
...
const getSayCount = (_participantId: number): number => {
  return 0;
};
