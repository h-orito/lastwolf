<template>
  <Modal v-model="isModalOpen" title="村の設定">
    ...
    <!-- フッター -->
    <template #footer>
      <UiButton color="secondary" variant="outline" @click="handleClose"> 閉じる </UiButton>
    </template>
  </Modal>
</template>
...
const settings = computed<SettingItem[]>(() => {
  ...
  items.push({
    name: "昼間時間",
    value: formatInterval(timeSetting.noon_seconds),
    description: "昼間フェーズの時間です。",
  });
  items.push({
    name: "投票時間",
    value: formatInterval(timeSetting.vote_seconds),
    description: "投票フェーズの時間です。",
  });
  items.push({
    name: "夜間時間",
    value: formatInterval(timeSetting.night_seconds),
    description: "夜間フェーズの時間です。",
  });
  ...
  if (isPrologueOrCancel.value) {
    ...
    // それ以外は現在の人数の編成のみ表示
    const participantCount = v.participants.count;
...
const addRuleSettings = (items: SettingItem[], rules: VillageRules) => {
  ...
  items.push({
    ...
    description: "「公開」の場合、死亡した人は全員の役職を知ることができます。",
  });
  ...
  items.push({
    name: "連続護衛",
    value: rules.available_same_target_guard ? "可能" : "不可",
    description:
