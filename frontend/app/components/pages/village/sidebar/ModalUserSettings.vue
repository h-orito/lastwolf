<template>
  <Modal v-model="isModalOpen" title="ユーザ設定">
    <div class="space-y-6">
      <!-- ページ分割 -->
      <section>
        <h4 class="mb-3 text-sm font-semibold text-gray-700 dark:text-gray-300">ページ分割</h4>
        <div class="space-y-3">
      ...
      <!-- 発言表示 -->
      <section>
        <h4 class="mb-3 text-sm font-semibold text-gray-700 dark:text-gray-300">発言表示</h4>
        <div class="space-y-3">
          <FormSwitch v-model="localSettings.isDispDate" label="日付を表示する" />
          <FormSwitch v-model="localSettings.isCharLarge" label="文字を大きく表示する" />
          <FormSwitch v-model="localSettings.isImgLarge" label="キャラ画像を大きく表示する" />
        </div>
      </section>

      <!-- テーマ -->
      <section>
        <h4 class="mb-3 text-sm font-semibold text-gray-700 dark:text-gray-300">テーマ</h4>
        <FormSwitch v-model="localSettings.isDarkTheme" label="ダークテーマにする" />
      </section>

      <!-- 操作 -->
      <section>
        <h4 class="mb-3 text-sm font-semibold text-gray-700 dark:text-gray-300">操作</h4>
        <div class="space-y-3">
          <FormSwitch v-model="localSettings.isOpenFilterNewtab" label="個人抽出を別タブで開く" />
          <FormSwitch
      ...
      <!-- 通知（参加者のみ） -->
      <section v-if="myself">
        <h4 class="mb-3 text-sm font-semibold text-gray-700 dark:text-gray-300">通知</h4>
        <div class="space-y-4">
    ...
    <template #footer>
      ...
      <UiButton color="primary" :loading="saving" @click="save"> 保存する </UiButton>
    </template>
  </Modal>
</template>
