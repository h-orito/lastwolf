  ...
  <div class="space-y-4">
    ...
    <div class="overflow-x-auto">
      <table class="min-w-full border-collapse text-sm">
        ...
        <tbody>
          <template v-for="(matome, lineIndex) in matomes.matomes" :key="lineIndex">
            <!-- データ行 -->
            <tr class="border-b border-gray-200 dark:border-gray-700">
              ...
              >
                <button
                  ...
                  :class="
                    selectingLineIndex === lineIndex && selectingContentIndex === contentIndex
            ...
            <!-- 記号パレット行（選択中の行にのみ表示） -->
            <tr v-if="selectingLineIndex === lineIndex" class="bg-gray-50 dark:bg-gray-800/50">
              <td></td>
        ...
        <tfoot>
          ...
          <tr>
            <td></td>
            <td v-for="(_, index) in matomes.charaNames" :key="index" class="px-1 py-1 text-center">
...
const emit = defineEmits<{
  ...
  "change-content": [{ lineIndex: number; contentIndex: number; content: string }];
}>();
...
const updateContent = (content: string) => {
  if (selectingLineIndex.value !== null && selectingContentIndex.value !== null) {
    emit("change-content", {
