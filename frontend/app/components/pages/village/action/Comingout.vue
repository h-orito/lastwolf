<template>
  <ActionPanel title="カミングアウト" panel-key="comingout">
    ...
    <!-- CO対象選択（2つまで） -->
    <p class="text-sm font-bold text-gray-700 dark:text-gray-300">CO対象（2つまで）</p>
    ...
    <div class="flex justify-end">
      <UiButton :disabled="!canSubmit" :loading="submitting" @click="handleComingout">
        カミングアウトする
      </UiButton>
    </div>
  </ActionPanel>
...
// Composables
const { submitting, error: comingoutError, comingout, clearError } = useComingout();
...
// カミングアウト可能かどうか
const canComingout = computed(() => comingoutSituation.value?.available_coming_out ?? false);

// 現在のカミングアウトのテキスト
const currentComingoutText = computed(() => {
  const current = comingoutSituation.value?.current_coming_out;
  const coList = current ? [current] : [];
  if (coList.length === 0) return "なし";
  return coList.map((co) => co.skill.name).join("と");
});

// 選択可能な役職リスト
const selectableSkills = computed(() => comingoutSituation.value?.selectable_skill_list ?? []);
...
// 初回読み込み時に初期化
watch(
  () => comingoutSituation.value?.current_coming_out,
  () => {
    initializeSelection();
  },
  { immediate: true },
...
const isSameAsCurrentComingout = computed(() => {
  const cur = comingoutSituation.value?.current_coming_out;
  const currentList = cur ? [cur] : [];
  ...
  return !selectedCodes.some((code) => !currentList.some((current) => current.skill.code === code));
});
