...
// Composables
const { submitting, error, actionSay, actionSayConfirm, clearError } = useActionSay();
...
const myselfText = computed(() => {
  ...
  return `${myself.value.chara.name.name}は、`;
});

// 生存状態
const isAlive = computed(() => myself.value?.dead == null);

// エピローグかどうか（VillageStatusにis_epilogueはないためfalse固定）
...
const targetOptions = computed(() => {
  ...
  // 自分以外の参加者を追加
  const otherParticipants = allParticipants.value.filter((p) => p.id !== myself.value?.id);
  otherParticipants.forEach((p) => {
    const name = p.chara.name.name;
...
const remainingCount = computed((): number | null => null);
const maxLength = computed(() => 200);

// 現在の文字数
const charCount = computed(() => message.value.length);

// 制限超過判定
const isCountExceeded = computed(() => remainingCount.value != null && remainingCount.value <= 0);
...
const createActionSayBody = () => ({
  myself: `${myself.value?.chara.name.name ?? ""}は、`,
