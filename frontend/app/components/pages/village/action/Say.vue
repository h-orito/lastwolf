<template>
  <ActionPanel id="say-panel" title="発言" panel-key="say">
    ...
    <div v-if="myself" class="mb-4 text-sm">
      <Alert v-if="myself.dead" type="info" class="mb-2"> あなたは死亡しました。 </Alert>
      <Alert v-if="participantStatus" type="warning" class="mb-2">
        {{ participantStatus }}
      </Alert>
      <Alert v-if="myself.skill" type="default" class="mb-2 whitespace-pre-line">
        {{ skillDescription }}
      </Alert>
      <span class="font-bold"
        >[{{ myself.chara.name.short_name }}] {{ myself.chara.name.name }}</span
      >
    ...
    <div v-if="replyTargetMessage" class="mt-4">
      <div class="rounded-lg border-2 border-blue-500 p-3">
        <div class="mb-2 flex items-center justify-between">
          <span class="text-xs font-bold text-blue-600 dark:text-(--ui-primary)">返信対象</span>
          <button
            type="button"
...
const SayConfirmModal = defineAsyncComponent(() => import("./say/SayConfirmModal.vue"));
...
const { setHasInputText, setConfirmModalOpen, setSubmitting, reset } = useVillageSayStatus();
...
const isProgress = computed(
  () => village.value?.status.is_recruiting === true || village.value?.status.is_progress === true,
);
...
const remainingMessageCount = computed((): number | null => null);
const maxMessageLength = computed(() => 200);
...
// 現在の文字数（改行を除く）
const currentCharCount = computed(() => messageText.value.replace(/\n/g, "").length);

// 制限超過判定
const isCountExceeded = computed(
  () => remainingMessageCount.value != null && remainingMessageCount.value <= 0,
);
const isLineExceeded = computed(() => currentLineCount.value > maxLineCount.value);
const isCharExceeded = computed(() => currentCharCount.value > maxMessageLength.value);

// Situationから取得する自分の参加者情報
const myself = computed(() => situation.value?.participate.myself);

// LASTWOLF APIには恋人システムがないため常にnull
const participantStatus = computed((): string | null => null);
...
const getDefaultFaceType = (_messageType: string): string => "NORMAL";
...
const insertAnchor = (anchorString: string) => {
  ...
  // カーソル位置にアンカーを挿入
  messageText.value =
    currentText.slice(0, selectionStart) + anchorString + currentText.slice(selectionEnd);
...
const initializeForm = () => {
  ...
  if (availableMessageTypes.value.length > 0) {
    ...
    if (firstMessageType) {
      selectedMessageType.value = firstMessageType.code;
      // 発言種別に応じた表情を設定
      selectedFaceType.value = getDefaultFaceType(firstMessageType.code);
    }
  }
};
...
const restoreDraft = () => {
  ...
  const isTypeAvailable = availableMessageTypes.value.some((m) => m.code === draft.messageType);
  if (isTypeAvailable) {
    selectedMessageType.value = draft.messageType;
  }
  nextTick(() => {
    selectedFaceType.value = "NORMAL";
  });
};
...
watch(
  ...
  () => {
    ...
    if (
      ...
      !availableMessageTypes.value.some((m) => m.code === selectedMessageType.value)
    ) {
      initializeForm();
    }
