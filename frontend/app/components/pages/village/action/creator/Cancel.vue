<template>
  <div>
    <!-- セクションタイトル -->
    <h3 class="mb-3 text-sm font-bold text-gray-900 dark:text-gray-100">廃村</h3>
    ...
    <Modal v-model="showConfirmModal" title="廃村確認">
      <p class="text-sm text-gray-700 dark:text-gray-300">本当に廃村にしますか?</p>

      <template #footer>
        <UiButton color="secondary" variant="outline" @click="showConfirmModal = false">
          キャンセル
        </UiButton>
        <UiButton :loading="submitting" :disabled="submitting" color="error" @click="handleCancel">
          廃村にする
        </UiButton>
      </template>
    </Modal>
...
// Composables
const { submitting, error: cancelError, cancelVillage, clearError } = useCancelVillage();
