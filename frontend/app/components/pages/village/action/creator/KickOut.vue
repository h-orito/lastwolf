<template>
  <div>
    <!-- セクションタイトル -->
    <h3 class="mb-3 text-sm font-bold text-gray-900 dark:text-gray-100">強制退村</h3>
    ...
    <Modal v-model="showConfirmModal" title="強制退村確認">
      ...
      <template #footer>
        <UiButton color="secondary" variant="outline" @click="showConfirmModal = false">
          キャンセル
        </UiButton>
        <UiButton :loading="submitting" :disabled="submitting" color="error" @click="handleKick">
          退村させる
        </UiButton>
      </template>
    </Modal>
...
const selectedParticipantName = computed(() => {
  ...
  const participant = allParticipants.value.find((p) => p.id === selectedParticipantId.value);
...
watch(
  participantOptions,
  (options) => {
    if (options.length > 0 && selectedParticipantId.value === null && options[0]) {
