  ...
  <Modal v-model="isOpen" title="画像から選択">
    <div class="flex flex-wrap justify-center gap-3">
      ...
      >
        ...
        <p class="mt-1 text-center text-xs">{{ participant.chara.name.name }}</p>
      </button>
    </div>
    <template #footer>
      <UiButton variant="outline" color="secondary" @click="close"> キャンセル </UiButton>
    </template>
  </Modal>
</template>
...
interface Props {
  modelValue: boolean;
  participantList: readonly VillageParticipantView[];
}
