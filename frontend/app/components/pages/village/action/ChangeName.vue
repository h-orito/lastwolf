<template>
  <ActionPanel title="名前変更" panel-key="change-name">
    ...
    <FormGroup label="キャラ名1文字略称" class="mb-3">
      <FormInput v-model="form.shortName" :maxlength="1" placeholder="1文字" size="sm" />
    </FormGroup>
...
watchEffect(() => {
  ...
  if (myself) {
    form.name = myself.chara.name.name;
    form.shortName = myself.chara.name.short_name;
...
const currentName = computed(() => {
  const myself = situation.value?.participate?.myself;
  if (!myself) return "-";
  return `[${myself.chara.name.short_name}] ${myself.chara.name.name}`;
});
...
const isSameAsCurrent = computed(() => {
  ...
  return form.name === myself.chara.name.name && form.shortName === myself.chara.name.short_name;
});
