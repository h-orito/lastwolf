<template>
  <div class="dark flex h-dvh w-[280px] flex-col overflow-hidden bg-[#363636] text-white">
    <!-- ヘッダー -->
    ...
    >
      <UiButton color="secondary" variant="solid" size="sm" @click="handleReset">
        リセット
      </UiButton>
      <UiButton color="primary" size="sm" :disabled="!canFilter" @click="handleFilter">
        抽出する
      </UiButton>
    </div>
  </div>
...
import { ALL_MESSAGE_TYPE_GROUPS, type MessageTypeGroup } from "~/lib/api/message-constants";
...
// State - フィルタ条件
const selectedMessageTypeGroups = ref<MessageTypeGroup[]>([...ALL_MESSAGE_TYPE_GROUPS]);
...
const canFilter = computed(() => {
  return selectedMessageTypeGroups.value.length > 0 && selectedParticipantIds.value.length > 0;
});
...
watch([storeMessageTypeGroups, storeParticipantIds, storeToParticipantIds, storeKeyword], () => {
  initializeFilter();
});
...
const handleFilter = () => {
  ...
  applyFilter(messageTypeGroups, participantIdList, toParticipantIdList, keyword.value);
