...
describe("message-converter", () => {
  ...
  describe("createAnchorCopyString", () => {
    it("should return anchor only for WEREWOLF_SAY", () => {
      expect(createAnchorCopyString(MESSAGE_TYPE.WEREWOLF_SAY, ">>*123", "Alice")).toBe(">>*123");
    });

    it("should prepend shortName for other types", () => {
      expect(createAnchorCopyString(MESSAGE_TYPE.NORMAL_SAY, ">>123", "Bob")).toBe("Bob>>123");
    });

    it("should prepend shortName for MONOLOGUE_SAY", () => {
      expect(createAnchorCopyString(MESSAGE_TYPE.MONOLOGUE_SAY, ">>-456", "Carol")).toBe(
        "Carol>>-456",
      );
    });
  });
  ...
  describe("convertToDecoratedText", () => {
    ...
    it("should convert bold tags", () => {
      expect(convertToDecoratedText("[[b]]bold text[[/b]]")).toBe("<strong>bold text</strong>");
    });
    ...
    it("should convert ruby tags", () => {
      expect(convertToDecoratedText("[[ruby]]漢字[[rt]]かんじ[[/rt]][[/ruby]]")).toBe(
  ...
  describe("getComingOutString", () => {
    ...
    it("should return multiple CO string", () => {
      const message = {
        from: {
          comming_outs: {
            list: [{ skill: { short_name: "占" } }, { skill: { short_name: "霊" } }],
          },
        },
      } as unknown as MessageView;
