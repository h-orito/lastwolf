...
export const convertToDecoratedText = (text: string): string => {
  ...
  t = t.replace(strikeRegex, '<span style="text-decoration: line-through;">$1</span>');
...
// アンカー番号の取得
export const getAnchorNum = (text: string): number => parseInt(RegExp(/\d{1,5}/).exec(text)![0]);

// アンカー表示判定
export const isDispAnchor = (isProgress: boolean, sayType: string): boolean => {
  // 独り言、秘話以外なら表示
  if (sayType !== MESSAGE_TYPE.MONOLOGUE_SAY && sayType !== MESSAGE_TYPE.SECRET_SAY) {
    return true;
  }
  // 独り言、秘話は進行中なら表示しない
  return !isProgress;
...
// メッセージ番号の計算（日付形式）
export const getMessageNumber = (message: DeepReadonly<MessageView> | MessageView): string => {
  return `${message.time.day}.0`;
};

// カミングアウト文字列の生成（LASTWOLF未対応）
export const getComingOutString = (
  _message: DeepReadonly<MessageView> | MessageView,
): string | null => {
  return null;
};
