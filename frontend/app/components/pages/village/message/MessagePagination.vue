<template>
  ...
  >
    ...
    <nav class="flex flex-1 items-center justify-end gap-1" aria-label="ページ">
      ...
      <template v-for="page in visiblePages" :key="page">
        <button
          ...
          :aria-current="!isLatestActive && page === effectiveCurrentPage ? 'page' : undefined"
    ...
    <!-- 最新ボタン -->
    <button type="button" :class="latestButtonClass" @click="$emit('disp-latest')">最新</button>
  </div>
</template>
...
const lastButtonClass = computed(() => {
  const isDisabled =
    ...
    (!props.isLatestActive && effectiveCurrentPage.value === props.messages.all_page_count);
  if (isDisabled) {
    return isDarkTheme.value
      ? `${baseButtonClass} cursor-not-allowed border-gray-700 bg-gray-800 text-gray-600`
      : `${baseButtonClass} cursor-not-allowed border-gray-200 bg-gray-100 text-gray-400`;
