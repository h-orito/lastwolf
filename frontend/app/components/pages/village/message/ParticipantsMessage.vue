<template>
  <div class="participants-message rounded border border-gray-300 p-2 sm:p-3 dark:border-gray-600">
      ...
      >
        ...
        <div class="flex-1" :class="textSizeClass">
          ...
          <div class="mb-1 flex flex-wrap items-center gap-1">
            ...
            <span class="ml-auto text-xs" :class="getCharaStatusClass(participant)">
          ...
          <div class="flex flex-wrap items-center gap-1">
            ...
            <span class="text-gray-600 dark:text-gray-400">{{ getSkillRequest(participant) }}</span>
            <span
              v-if="participant.winlose"
...
// 参加者の並び順（死亡者が先、同じ死亡状況なら日付順、同日なら死因順）
const sortedParticipants = computed(() => {
  const members = [...allParticipants.value];
...
// 参加者型（読み取り専用も許容）
type ParticipantType = DeepReadonly<VillageParticipantView> | VillageParticipantView;

// キャラクター名（長い場合は省略）
const getCharaName = (participant: ParticipantType): string => {
  const fullName = participant.chara.name.name;
...
const getCharaStatusClass = (participant: ParticipantType): string => {
  ...
  const reason = participant.dead.reason;
  if (reason === "突然" || reason === "処刑") return "text-blue-600 dark:text-(--ui-primary)";
  return "text-red-500 dark:text-red-400";
};

// 勝敗状態
const getWinStatus = (participant: ParticipantType): string => {
  return participant.winlose?.name ?? "";
};

// 勝敗状態のスタイルクラス
const getWinStatusClass = (participant: ParticipantType): string => {
  if (!participant.winlose) return "";
  return participant.winlose.code === "WIN"
