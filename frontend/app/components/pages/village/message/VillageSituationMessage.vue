<template>
  <div v-if="isCurrentVillageDayLatest" class="text-xs leading-4.5">
    ...
    <!-- 沈黙時間メッセージ -->
    <UiFeedbackAlert v-if="isSilentTime" :class="charSizeClass" class="mb-1.5" type="warning">
...
const prologueMessage = computed(() => {
  ...
  const currentParticipantCount = village.value.participants.count;
...
const progressMessage = computed(() => {
  ...
  const dayList = village.value.days.list;
...
const daychangeDatetime = computed(() => {
  ...
  const latestDayData = dayList.at(-1);
  if (!latestDayData) return "";
  return latestDayData.end_datetime;
});
...
// 沈黙時間かどうか（LASTWOLFには未対応）
const isSilentTime = computed(() => false);

// 突然死警告メッセージ
const suddenlyDeathMessage = computed(() => {
  const noSayMemberNames = noSayMembers.value.map((member) => member.chara.name.name);
...
// 未発言メンバー（LASTWOLFにtoday_message_count_map未対応のため常に空）
...
// 発言回数取得（LASTWOLFに未対応のため常に0以外を返す → 突然死警告は出ない）
