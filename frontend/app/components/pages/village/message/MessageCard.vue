<template>
  <div :id="`mc-${message.time.unix_time_milli}`" class="font-normal">
    ...
    <!-- システムメッセージ -->
    <SystemMessage v-else-if="isSystemType" :message="message" :is-large-text="isLargeText" />
...
const isSystemType = computed(() => checkSystemType(props.message.content.type.code));
const isActionType = computed(() => checkActionType(props.message.content.type.code));
const isParticipantsType = computed(() => checkParticipantsType(props.message.content.type.code));
</script>
