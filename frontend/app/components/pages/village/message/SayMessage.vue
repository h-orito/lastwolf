  ...
  <div class="say-message">
    ...
    <div class="mb-1 flex flex-wrap items-start gap-x-2 text-xs">
      ...
      <div class="flex min-w-0 items-center gap-1">
        ...
        <!-- カミングアウト -->
        <span v-if="comingOutString" class="rounded border border-gray-300 px-1 py-0.5 text-xs">
          {{ comingOutString }}
        </span>
    ...
    <div class="flex gap-2">
      ...
      <div class="flex-1">
        ...
        <div
          ...
          :class="[messageClass, isLargeText ? 'text-sm leading-5.5' : 'text-xs leading-4.5']"
...
const maxCount = computed((): number | null => null);
...
const imageWidth = computed(() => {
  const baseWidth = props.message.from?.chara?.image.width || 60;
  return props.isImgLarge ? Math.floor(baseWidth * 1.5) : baseWidth;
});

const imageHeight = computed(() => {
  const baseHeight = props.message.from?.chara?.image.height || 80;
...
// アンカー文字列（LASTWOLFにnum未対応のため常に空）
const anchorString = computed(() => "");

// アンカーコピー文字列
const anchorCopyString = computed(() => {
  const shortName = props.message.from?.chara.name.short_name || "";
  return createAnchorCopyString(props.message.content.type.code, anchorString.value, shortName);
});

// キャラクター名
const characterName = computed(() => {
  const shortName = props.message.from?.chara.name.short_name || "";
  const name = props.message.from?.chara.name.name || "";
...
// 秘話の宛先キャラクター名（LASTWOLFにto未対応のため常にnull）
const targetCharacterName = computed((): string | null => null);

// カミングアウト文字列
const comingOutString = computed(() => getComingOutString(props.message));

// プレイヤー情報
const twitterUserName = computed(() => props.message.from?.player?.twitter_user_name);
...
// フォーマット済みメッセージテキスト
const formattedMessageText = computed(() => convertToMessageText(props.message.content.text));
...
const clickAnchorMessage = async (anchorString: string): Promise<void> => {
  ...
  // 既に表示中ならトグルで非表示に
  const existingIndex = anchorMessages.value.findIndex((mes) => mes.content.type.code === typeCode);
  if (existingIndex >= 0) {
