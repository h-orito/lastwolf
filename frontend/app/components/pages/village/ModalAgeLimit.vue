<template>
  ...
  >
    <p>
      この村は年齢制限が
      <strong class="text-lg text-red-500 dark:text-red-400">{{ ageLimit }}</strong>
      に設定されており、<br />
      暴力表現{{ ageLimit === "R18" ? "や性描写" : "" }}などが含まれる可能性があります。
    </p>
...
// 年齢制限タグ（LASTWOLFには未対応）
const ageLimit = computed<string | null>(() => null);
