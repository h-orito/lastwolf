  ...
  <div class="space-y-6 text-left">
    ...
    <section>
      ...
      <div class="mb-2 flex gap-2 text-sm">
        <UiButton size="xs" variant="link" class="p-0" @click="handleAllMessageTypeOn">
          全てON
        </UiButton>
        <span>/</span>
        <UiButton size="xs" variant="link" class="p-0" @click="handleAllMessageTypeOff">
          全てOFF
        </UiButton>
        <span>/</span>
        <UiButton size="xs" variant="link" class="p-0" @click="handleReverseMessageType">
          反転
        </UiButton>
      </div>
      ...
      <div class="space-y-2">
        <FormCheckGroup
          ...
          @update:model-value="$emit('update:selectedMessageTypeGroups', $event)"
        />
        <FormCheckGroup
          :model-value="selectedMessageTypeGroups"
    ...
    <section>
      ...
      <div class="mb-2 flex gap-2 text-sm">
        <UiButton size="xs" variant="link" class="p-0" @click="handleAllParticipantOn">
          全てON
        </UiButton>
        <span>/</span>
        <UiButton size="xs" variant="link" class="p-0" @click="handleAllParticipantOff">
          全てOFF
        </UiButton>
        <span>/</span>
        <UiButton size="xs" variant="link" class="p-0" @click="handleReverseParticipant">
          反転
        </UiButton>
      </div>
        ...
        >
          ...
          >
            <div class="flex items-center gap-2">
              ...
              <span class="text-xs">{{ participant.chara.name.name }}</span>
            </div>
          </FormCheckbox>
        </div>
      </div>
    ...
    <section>
      ...
      <div class="mb-2 flex gap-1 text-sm">
        <UiButton size="xs" variant="link" class="p-0" @click="handleAllToParticipantOn">
          全てON
        </UiButton>
        <span>/</span>
        <UiButton size="xs" variant="link" class="p-0" @click="handleAllToParticipantOff">
          全てOFF
        </UiButton>
        <span>/</span>
        <UiButton size="xs" variant="link" class="p-0" @click="handleReverseToParticipant">
          反転
        </UiButton>
        <template v-if="myself">
          <span>/</span>
          <UiButton size="xs" variant="link" class="p-0" @click="handleToMyParticipant">
            自分宛
          </UiButton>
        </template>
      </div>
...
const handleReverseMessageType = () => {
  emit(
    ...
    ALL_MESSAGE_TYPE_GROUPS.filter((type) => !props.selectedMessageTypeGroups.includes(type)),
  );
};
...
const toggleParticipant = (id: number, checked: boolean | string) => {
  ...
  if (isChecked) {
    if (!props.selectedParticipantIds.includes(id)) {
      emit("update:selectedParticipantIds", [...props.selectedParticipantIds, id]);
    }
  } else {
    emit(
...
const handleReverseParticipant = () => {
  emit(
    "update:selectedParticipantIds",
    allParticipantIds.value.filter((id) => !props.selectedParticipantIds.includes(id)),
  );
};
...
const toggleToParticipant = (id: number, checked: boolean | string) => {
  ...
  if (isChecked) {
    if (!props.selectedToParticipantIds.includes(id)) {
      emit("update:selectedToParticipantIds", [...props.selectedToParticipantIds, id]);
    }
  } else {
    emit(
...
const handleReverseToParticipant = () => {
  emit(
    "update:selectedToParticipantIds",
    allParticipantIds.value.filter((id) => !props.selectedToParticipantIds.includes(id)),
  );
};
