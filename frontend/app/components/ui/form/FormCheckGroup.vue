...
const selectNext = (currentValue: T) => {
  ...
  const currentIndex = props.options.findIndex((opt) => opt.value === currentValue);
  const nextIndex = currentIndex === -1 ? 0 : (currentIndex + 1) % props.options.length;
  const nextOption = props.options[nextIndex];
  if (nextOption) {
    const nextButton = document.getElementById(`${computedId.value}-${nextOption.value}`);
    nextButton?.focus();
  }
};
...
const selectPrevious = (currentValue: T) => {
  ...
  if (prevOption) {
    const prevButton = document.getElementById(`${computedId.value}-${prevOption.value}`);
    prevButton?.focus();
  }
};
...
const getTabIndex = (value: T): number => {
  ...
  if (props.modelValue.length === 0 && props.options[0]?.value === value) return 0;
...
const buttonClasses = (value: T, index: number) => {
  ...
  return [baseClasses, roundedClasses, borderClasses, stateClasses, disabledClasses].join(" ");
};
</script>
