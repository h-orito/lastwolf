<template>
  <BaseModal :model-value="isOpen" title="画像から選択" @close="handleClose">
    <p id="modal-description" class="sr-only">キャラクター一覧から選択してください</p>
    ...
    <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
      ...
      >
        <img
          ...
          loading="lazy"
        />
        <div v-else class="mb-2 flex h-20 w-20 items-center justify-center bg-gray-100">
          <Icon name="account" class="text-3xl text-gray-400" />
        </div>
        <p class="text-center text-xs">{{ chara.name.name }}</p>
      </button>
    </div>
  </BaseModal>
</template>
