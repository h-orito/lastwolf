 ...
 * 村の発言フィルタ状態管理Store
 */
export const useVillageMessageFilterStore = defineStore("village-message-filter", () => {
  // Filter state
  const messageTypeGroups = ref<MessageTypeGroup[]>([...ALL_MESSAGE_TYPE_GROUPS]);
  ...
  const setMessageFilter = (filter: {
    ...
    if (filter.messageTypeGroups !== undefined) {
      messageTypeGroups.value = filter.messageTypeGroups ?? [...ALL_MESSAGE_TYPE_GROUPS];
    }
