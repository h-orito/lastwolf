...
export const useVillageNavigation = () => {
   ...
   * カスタムスムーズスクロール（高速版）
   */
  const smoothScroll = (container: Element, targetTop: number, duration: number) => {
   ...
   * ページ最上部にスクロール
   */
  const scrollToTop = (containerSelector: string = "#village-article-wrapper") => {
   ...
   * ページ最下部にスクロール
   */
  const scrollToBottom = (containerSelector: string = "#village-article-wrapper") => {
    const container = document.querySelector(containerSelector);
    if (!container) return;
