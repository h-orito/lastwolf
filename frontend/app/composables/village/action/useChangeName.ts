...
export const useChangeName = () => {
  ...
  /**
   * 名前変更API
   * POST /village/{villageId}/change-name
   * @param name 新しい名前
   * @param shortName 新しい1文字略称
   * @returns 成功時true、失敗時false
   */
  const changeName = async (name: string, shortName: string): Promise<boolean> => {
  ...
  const handleApiError = (err: unknown) => {
    ...
    console.error("Change name API error:", err);
  };
