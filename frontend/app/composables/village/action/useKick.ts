...
export const useKick = () => {
  ...
  /**
   * 強制退村実行API
   * POST /creator/village/{villageId}/kick
   * @param targetId 退村対象の参加者ID
   * @returns 成功時true、失敗時false
   */
  const kick = async (targetId: number): Promise<boolean> => {
    ...
    try {
      ...
      await apiCall(`/creator/village/${villageId.value}/kick`, {
        method: "POST",
        body,
      });
      return true;
