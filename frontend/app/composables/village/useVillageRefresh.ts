...
export const useVillageRefresh = () => {
  ...
  // Composables
  const { loadVillage, villageId, latestDay, isCurrentVillageDayLatest, changeCurrentVillageDay } =
    useVillage();
  const { loadMessages, isDispLatest, isViewingLatestMessages, setDispLatest } = useMessage();
