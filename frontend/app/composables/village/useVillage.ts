import type { VillageView, VillageDayView, CharachipView, CharachipsView } from "~/lib/api/types";
...
export const useVillage = () => {
  ...
  const fetchCharachips = async (): Promise<CharachipView[]> => {
    try {
      ...
      return response.list.filter((c) => c.id === charachipId);
    } catch (err) {
  ...
  const loadVillage = async () => {
    ...
    try {
      const data = await apiCall<VillageView>(`/village/${villageStore.villageId}`);
      villageStore.saveVillage(data);
    } catch (err) {
      error.value =
        err instanceof Error
          ? err
          : new Error(`村情報の取得に失敗しました (村ID: ${villageStore.villageId})`);
      console.error(`村情報の取得に失敗しました (村ID: ${villageStore.villageId}):`, err);
    } finally {
      loading.value = false;
    }
  };
  ...
  const changeCurrentVillageDayById = (villageDayId: number) => {
    ...
    const day = villageStore.village.days.list.find((d) => d.id === villageDayId);
    if (day) {
      villageStore.saveCurrentVillageDay(day);
    }
  };
  ...
  const allParticipants = computed(() => {
    if (!villageStore.village) return [];
    return villageStore.village.participants.member_list;
  });
  ...
  const currentVillageDayIndex = computed(() => {
    ...
    return villageStore.village.days.list.findIndex(
  ...
  const existNextDay = computed(() => {
    if (!villageStore.village || !villageStore.currentVillageDay) return false;
    return currentVillageDayIndex.value !== villageStore.village.days.list.length - 1;
  });
  ...
  const prevDayId = computed(() => {
    ...
    return villageStore.village.days.list[currentVillageDayIndex.value - 1]?.id ?? null;
  });
