...
export const useMessage = () => {
  ...
  const { messageTypeGroups, messageTypes, participantIds, toParticipantIds, keyword } =
  ...
  const loadMessages = async () => {
    ...
    try {
      ...
      // クエリパラメータの構築
      const params: Record<string, string | number | string[] | number[] | boolean> = {};
      ...
      const allParticipantCount = village.value ? village.value.participants.member_list.length : 0;
      if (
      ...
      if (pagingSettings.isPaging && pagingSettings.messagePerPage) {
        ...
        params.page_num = messageStore.isDispLatest ? 10000 : messageStore.currentPageNum;
        params.is_disp_latest = messageStore.isDispLatest;
      }

      // URLの構築(配列パラメータはrepeat形式で送信)
      const day = currentVillageDay.value;
      const url = `/village/${villageId.value}/day/${day.day}/time/${day.noon_night.code}/message-list`;
  ...
  const isViewingLatestMessages = computed(() => {
    return (
      ...
      messageStore.messages?.current_page_num === messageStore.messages?.all_page_count
    );
  });
