...
export const useUserSettings = () => {
  ...
  /** ページング設定 */
  const paging = computed<PagingSettings>(() => settingsStore.settings?.paging ?? DEFAULT_PAGING);
  ...
  /** テーマ設定 */
  const theme = computed<ThemeSettings>(() => settingsStore.settings?.theme ?? DEFAULT_THEME);
  ...
  const ageLimit = computed<AgeLimitSettings>(() => ({
    confirmVillageIds: [...(settingsStore.settings?.ageLimit?.confirmVillageIds ?? [])],
  }));

  /** 固定中のパネルキー */
  const fixedPanelKey = computed<string | null>(() => actionWindow.value.fixedPanelKey ?? null);
   ...
   * メッセージ表示設定を保存
   */
  const setMessageDisplay = (messageDisplaySettings: MessageDisplaySettings) => {
   ...
   */
  const getActionPanelOpen = (panelKey: string, defaultOpen: boolean = true): boolean => {
    const openMap = actionWindow.value.openMap;
