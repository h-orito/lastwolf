...
export const useCachedApi = () => {
   ...
   */
  const getCachedData = <T>(key: string, maxAge: number = DEFAULT_CACHE_MAX_AGE): T | undefined => {
  ...
  const fetchWithCache = async <T>(
    ...
    const { maxAge = DEFAULT_CACHE_MAX_AGE, withAuth = false, fetchOptions = {} } = options;
