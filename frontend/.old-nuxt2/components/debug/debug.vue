<template>
  <b-message type="is-dark" size="is-small" title="デバッグ" class="debug" :closable="false">
    <b-field>
      <b-button type="is-primary" size="is-small" @click="fetchDebugVillage">村取得</b-button>
    </b-field>
    <b-field label="ダミーログイン" label-position="on-border">
      <b-select v-model="playerId" :disabled="!village" expanded size="is-small">
        <option v-for="player in playerList" :value="player.player_id" :key="player.player_id">
          {{ player.name }}
        </option>
      </b-select>
      <p class="control">
        <button class="button is-primary is-small" :disabled="!village" @click="dummyLogin">
          でログインする
        </button>
      </p>
    </b-field>
    <b-field label="参加させる" label-position="on-border">
      <b-select v-model="participateCharaNum" :disabled="!isPrologue" expanded size="is-small">
        <option v-for="num in participateMemberNumList" :value="num" :key="num">
          {{ `${num}人` }}
        </option>
      </b-select>
      <p class="control">
    ...
    <b-field>
      <b-button type="is-primary" size="is-small" :disabled="!village" @click="allRollcall"
        >全員点呼</b-button
      >
    </b-field>
    <b-field>
      <b-button type="is-primary" size="is-small" :disabled="!village" @click="changeDay"
        >残り10秒にする</b-button
      >
    </b-field>
    <b-field label="指定処刑" label-position="on-border">
      <b-select v-model="participantId" :disabled="!isVoteTime" expanded size="is-small">
        <option v-for="participant in voteTargetList" :value="participant.id" :key="participant.id">
    ...
    <b-field>
      <p class="control">
        <b-button type="is-primary" size="is-small" :disabled="!isVoteTime" @click="allDrawVote"
          >引分投票</b-button
        >
      </p>
    </b-field>
    <b-field>
      <b-button type="is-danger" size="is-small" class="m-t-10" @click="allDelete"
        >メッセージ削除</b-button
      >
    </b-field>
  </b-message>
...
export default class Debug extends Vue {
  ...
  private get playerList(): Array<Player> {
    ...
    const list: Player[] = [];
    for (let i = 1; i <= 18; i++) {
      const participant = this.village.participants.member_list.find((p) => p.player!.id === i);
      list.push({
        name: participant ? this.dummyLoginCharaName(participant) : `未参加: player_id: ${i}`,
        player_id: i,
      });
    }
    return list;
  ...
  private async allVote(): Promise<void> {
    await this.$axios.$post(`/admin/village/${this.village!.id}/all-vote/${this.participantId}`);
  }

  private get isVoteTime(): boolean {
    return (
      !!this.village &&
      this.village.days.list[this.village.days.list.length - 1].noon_night.code.startsWith("VOTE")
    );
  }
}
