<template>
  <b-message type="is-dark" size="is-small" title="進行" class="myself" :closable="false">
...
@Component({
  components: {
    ...
    currentSituation: () => import("~/components/progress/current-situation.vue"),
    progressBar: () => import("~/components/progress/progress-bar.vue"),
    action: () => import("~/components/action/action.vue"),
    villageInfoModal: () => import("~/components/progress/modal-village-info.vue"),
  },
})
...
export default class Progress extends Vue {
  ...
  private async openVillageInfoModal(): Promise<void> {
    if (!this.charachipName) {
      const charachipId = this.$store.getters.village.setting.charachip.charachip_id;
