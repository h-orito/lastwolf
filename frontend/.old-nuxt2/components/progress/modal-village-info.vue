...
export default class ModalVillageInfo extends Vue {
  ...
  private addCharachipSetting(settings: Settings[]): void {
    ...
    const self = this;
    const dummyCharaName = this.village.participants.member_list.find((member) => {
  ...
  private addOrganizationSetting(settings: Settings[]): void {
    if (!this.village) return;
    const org: string =
      this.village.setting.organizations.organization[this.village.setting.capacity.min];
    settings.push({
      name: "編成",
      value: org,
  ...
  private addPasswordSetting(settings: Settings[]): void {
    ...
    settings.push({
      name: "入村パスワード",
      value: this.village.setting.password.join_password_required ? "あり" : "なし",
