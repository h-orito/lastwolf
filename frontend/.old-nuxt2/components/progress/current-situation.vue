<template>
  <p v-if="village">
    <span v-for="messageLine in currentSituationDescription" :key="messageLine.id">
...
export default class Progress extends Vue {
  ...
  private get currentSituationDescription(): string[] {
    if (!!this.village && isProgress(this.village)) {
      return (
        this.currentDay + createCurrentDescription(this.village, this.$store.getters.latestDay)
      )
        .replace(/\n/gm, "<br>")
        .split("<br>");
    } else {
      return createCurrentDescription(this.village, this.$store.getters.latestDay)
        .replace(/\n/gm, "<br>")
        .split("<br>");
    }
  }
...
const createCurrentDescription = (village: Village | null, latestDay: VillageDay): string => {
  if (!village) return "";
...
const isPrologue = (village: Village): boolean => village.status.code === VILLAGE_STATUS.PROLOGUE;

const isRollcalling = (village: Village): boolean =>
  village.status.code === VILLAGE_STATUS.ROLLCALLING;

const isProgress = (village: Village): boolean => village.status.code === VILLAGE_STATUS.PROGRESS;

const isEpilogue = (village: Village): boolean => village.status.code === VILLAGE_STATUS.EPILOGUE;

const isComplete = (village: Village): boolean => village.status.code === VILLAGE_STATUS.COMPLETE;

const isCanceled = (village: Village): boolean => village.status.code === VILLAGE_STATUS.CANCEL;
