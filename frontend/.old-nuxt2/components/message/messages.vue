<template>
  <b-message type="is-dark" size="is-small" title="チャット" class="messages" :closable="false">
    <b-tabs size="is-small" type="is-toggle" expanded multiline v-model="tabId" :animated="false">
      <b-tab-item
        v-for="day in days"
        :key="day.id"
        :value="day.id"
        ...
        icon-pack="fas"
      >
        <alive-participants @filter="filteringOrCancel" :day="day" :filtering-id="filteringId" />
        <div>
          <form-switch
            rules="required"
            label-message=""
...
export default class Messages extends Vue {
  private tabId: number = this.days.length === 0 ? 0 : this.days[this.days.length - 1].id;
  ...
  private get days(): VillageDay[] {
    ...
    return this.village.days.list.filter((d) => !d.noon_night.code.startsWith("VOTE"));
  }
