<template>
  <b-message type="is-dark" size="is-small" title="参加者" class="participants" :closable="false">
    <participant
    ...
    <div v-if="isViewablePlayerInfo">
      ...
      <b-button type="is-primary" size="is-small" @click="toggleDisplayPlayer">{{
...
export default class Participants extends Vue {
  ...
  private get isSolved(): boolean {
    return (
      !!this.village &&
      [VILLAGE_STATUS.EPILOGUE, VILLAGE_STATUS.COMPLETE, VILLAGE_STATUS.CANCEL].some(
        (s) => s === this.village!!.status.code,
      )
    );
  }
  ...
  private get isViewablePlayerInfo(): boolean {
    return this.isSolved || (!!this.situation && this.situation.creator.viewable_spoiler);
  }

  private get toggleDisplayButtonMessage(): string {
    return this.isDispPlayer ? "プレイヤー情報を表示しない" : "プレイヤー情報を表示する";
  }
