<template>
  <div class="participant has-text-centered m-b-10" :style="isDispPlayer ? 'width: 100%' : ''">
...
export default class ParticipantV extends Vue {
  ...
  private get isMyself(): boolean {
    const situation: SituationAsParticipant | null = this.$store.getters.situation;
  ...
  private get doneRollcall(): boolean {
    return (
      this.village.status.code === VILLAGE_STATUS.ROLLCALLING && this.participant.done_roll_call
    );
  }
