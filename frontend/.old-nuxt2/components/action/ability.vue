...
export default class Ability extends Vue {
  ...
  private get targetList(): VillageParticipant[] {
    return this.situation.ability.list.find((a) => a.type.code === this.abilityType)!!.target_list;
  }
  ...
  private get abilityMessage(): string[] {
    ...
    if (type === "GUARD" && !this.village.setting.rules.available_same_target_guard) {
  ...
  private confirmSetAbility(): void {
    const self = this;
    const target: VillageParticipant = this.targetList.find((p) => p.id === self.participantId)!!;
