  ...
  >
    <div class="modal-card">
      ...
      <footer class="modal-card-foot" style="justify-content: flex-end !important">
        <b-button type="is-secondary" size="is-small" @click="close"> 戻る </b-button>
        <b-button type="is-primary" size="is-small" @click="create" :disabled="submitting">
          {{ saveLabel }}
        </b-button>
      </footer>
    </div>
...
export default class ModalVillageInfo extends Vue {
  ...
  private addTimeSetting(settings: Settings[]): void {
    ...
    // @ts-ignore
    const start = this.$dayjs(timeParam.start_datetime.replace("T", " ")).format(
      "YYYY-MM-DD HH:mm",
    );
    settings.push({
      name: "開始日時",
