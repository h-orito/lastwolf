...
const actions = {
  async [actionType.INIT_VILLAGE]({ state, commit }, { villageId, uid, dayChangeCallback }) {
    if (villageRef) {
      villageRef.off();
...
const getters = {
  ...
  latestDay: (state) =>
    state.village ? state.village.days.list[state.village.days.list.length - 1] : null,
...
const fetchVillage: any = async (villageId, app, state, commit, dayChangeCallback) => {
