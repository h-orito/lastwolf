...
export default class VillageSetting extends Vue {
  ...
  private reset(): void {
    ...
    this.organization = Object.values(this.village.setting.organizations.organization).join("\n");
    ...
    this.silentSeconds = rules.silent_seconds == null ? "0" : rules.silent_seconds.toString();
    this.joinPassword = "";
  }

  private async confirmSetting({ param, errCb, successCb }): Promise<void> {
    try {
      await this.$axios.$post(`/village/${this.villageId}/setting/confirm`, param);
      successCb();
    } catch (error) {
      errCb(error);
    }
  ...
  private async modifySetting({ param, errCb }): Promise<void> {
    try {
      const res = await this.$axios.$post(`/village/${this.villageId}/setting`, param);
